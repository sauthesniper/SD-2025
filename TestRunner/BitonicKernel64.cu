...
__global__ void BitonicSort(int64_t* v, int size, int stage, int pass) {
	...
	int64_t minval = min(v[li], v[ri]);
	int64_t maxval = max(v[li], v[ri]);
	v[li] = minval;
	v[ri] = maxval;
}

void LaunchCudaSortKernel(int64_t* cpuData, int size) {
	int64_t* gpuData;
	...
	// Allocate on the gpu
	cudaMalloc((void**)&gpuData, size * sizeof(int64_t));
	auto end = high_resolution_clock::now();
	auto duration = duration_cast<milliseconds>(end - start);
	cout << "GPU memory allocation took " << duration.count() << "ms\n";

	start = high_resolution_clock::now();
	cudaMemcpy(gpuData, cpuData, size * sizeof(int64_t), cudaMemcpyHostToDevice);
	...
	start = high_resolution_clock::now();
	// Copy data from gpu
	cudaMemcpy(cpuData, gpuData, size * sizeof(int64_t), cudaMemcpyDeviceToHost);
	cudaFree(gpuData);
	end = high_resolution_clock::now();
	duration = duration_cast<milliseconds>(end - start);
...
int64_t* ReadNumbersFromFile(const char* filePath, int* size){
...
int main(int argc, char* argv[]){
	...
	for(int i = 1; i < argc; i++) {
		...
		int size;
		int64_t* arr = ReadNumbersFromFile(filePath, &size);
